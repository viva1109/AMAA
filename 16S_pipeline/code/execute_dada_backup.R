...
executedada = function(trimdir,outdir,ncore){
    ...
    filtRes = filterAndTrim(fnFs, filtFs,fnRs, filtRs, maxEE=c(3,5), rm.phix=TRUE, truncQ= 2,
                            truncLen=c(275,250),multithread=ncore, verbose=TRUE)
...
if(sstep<2){
    executedada(trimPath,alloutdir,ncore)
}
